...
export async function GET(request: Request) {
  ...
  if (!code) return NextResponse.redirect(`${getBaseUrl()}/settings?error=invalid_code`);

  try {
    const { tokens } = await auth.getToken(code);
    if (!tokens.id_token || !tokens.access_token || !tokens.refresh_token || !tokens.scope) {
      return NextResponse.redirect(`${getBaseUrl()}/settings?error=invalid_token`);
    }
    ...
    if (!details?.email) {
      return NextResponse.redirect(`${getBaseUrl()}/settings?error=invalid_email`);
    }
    ...
    return NextResponse.redirect(`${getBaseUrl()}/settings?tab=integrations`);
  } catch (error) {
    return NextResponse.redirect(`${getBaseUrl()}/settings?error=${error}`);
  }
}
