  ...
  list: mailboxProcedure.query(async ({ ctx }) => {
    ...
    try {
      const apis = await db.query.toolApis.findMany({
        ...
        where: eq(toolApis.unused_mailboxId, mailbox.id),
      });
  ...
  update: mailboxProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const tool = await db.query.tools.findFirst({
        where: and(eq(toolsTable.id, toolId), eq(toolsTable.unused_mailboxId, mailbox.id)),
      });
  ...
  deleteApi: mailboxProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await db.transaction(async (tx) => {
        await tx
          .delete(toolsTable)
          .where(and(eq(toolsTable.toolApiId, apiId), eq(toolsTable.unused_mailboxId, mailbox.id)));
        await tx.delete(toolApis).where(and(eq(toolApis.id, apiId), eq(toolApis.unused_mailboxId, mailbox.id)));
      });
  ...
  refreshApi: mailboxProcedure
    ...
    .mutation(async ({ ctx: { mailbox }, input: { apiId, schema } }) => {
      const api = await db.query.toolApis.findFirst({
        where: and(eq(toolApis.id, apiId), eq(toolApis.unused_mailboxId, mailbox.id)),
      });
